...
# for ii in range(1,2):
#     name = 'RollerCoaster_alpha_beta'
#     file = open('/Users/Chenge/Downloads/360videotrace/EPFL/RollerCoasterGroudTruth/'+name+'_'+str(ii)+'.mat','r')
#     temp = loadmat(file)[name]
#     plt.figure()
#     plt.scatter(range(temp.shape[0]),temp[:,0])
#     plt.scatter(range(temp.shape[0]),temp[:,1],color='r')
import glob as glob
import os
import numpy as np
...
if __name__ == '__main__':
    ...
    directory = os.path.join('/scratch/cl2840/360video/tsinghua/Formated_Data/',experimentpath)
    user_num = 48
    # video info
    videoMetaFile = os.path.join('/scratch/cl2840/360video/tsinghua/Formated_Data/',experimentpath,'videoMeta.csv')
