...
others_fut_inputs = Input(shape=(max_decoder_seq_length,(num_user-1),3*fps))
if mlp_mixing:
...
if ame:
    ...
    for user_ind in range(num_user):
        ...
        # lstm_pool.append(LSTM(latent_dim, stateful=cfg.stateful_across_batch, return_state=True, return_sequences=True))
...
for user_ind in range(num_user-1):
    # oth_states, state_h_oth, state_c_oth = lstm_pool[user_ind](get_dim2_layer(util.slice_layer(2,user_ind,user_ind+1)(others_fut_inputs)))
    oth_states, state_h_oth, state_c_oth = lstm(get_dim2_layer(util.slice_layer(2,user_ind,user_ind+1)(others_fut_inputs)))
    oth_states_list.append(expand_dim2_layer(oth_states))

oth_states_list = Lambda(lambda x: K.concatenate(x, axis=2))(oth_states_list)#(batch,10,33,64)
...
for time_ind in range(max_decoder_seq_length):
    ...
    if ame:
        ...
        for user_ind in range(num_user-1):
            ...
            # u_i_list.append(h_all)#no embedding, just the location sim
...
#use two losses
# model = Model([encoder_inputs, others_fut_inputs, decoder_inputs], [decoder_outputs,decoder_outputs])
...
if cfg.input_mean_var:
    ...
    # ### other users
    others_fut_input_data = util.get_gt_target_xyz_oth(_video_db_future_oth)
    ...
    ### other users
    others_fut_input_data = _video_db_future_oth.reshape((_video_db_future_oth.shape[0],_video_db_future_oth.shape[1],num_user-1,-1))
    # others_fut_input_data = util.get_gt_target_xyz_oth(_video_db_future_oth)
...
if cfg.sample_and_refeed or cfg.stateful_across_batch:
    ...
    others_fut_input_data = util.rand_sample(others_fut_input_data,sample_ind)
    # sanity check
...
# model.fit([encoder_input_data, others_fut_input_data, decoder_input_data], [decoder_target_data,decoder_target_data_muvar],#two losses
